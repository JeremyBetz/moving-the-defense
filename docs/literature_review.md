...
Esposito et al. (2026) provide a timely field-level constraint. Their scoping review of elite off-ball football tracking reports fragmented definitions, heterogeneous methods, simplified outcomes, and limited integration of opponents and situational context across 32 included studies. That supports the project's emphasis on transparent temporal units and claim separation, but it also raises the novelty bar: attacking movement episodes sit inside an active, methodologically diverse literature rather than an empty problem space.

A targeted post-prominence audit adds several relevant method families. Kai et al. (2021) derive direction, speed, acceleration, and jerk from football tracking to locate and measure change-of-direction intervals. Corbett, Sweeting, and Robertson (2019) apply binary change-point segmentation to team-sport velocity time series. General trajectory work distinguishes change-point, state-space, and geometric/path-simplification approaches; TRACLUS makes angular and perpendicular error explicit, while multivariate trajectory methods show how sustained changes can be located jointly across two spatial dimensions. These are precedents for the selected next candidate—penalized change points in $[v_x,v_y]$—not evidence that it is validated for attacking movement.

The frozen directional-segmentation protocol uses PELT only as an exact optimizer of a penalized objective (Killick, Fearnhead, and Eckley, 2012) and uses a transparent two-dimensional extension of Schwarz/BIC mean-change selection (Yao, 1988). Those sources support the algorithmic and penalty families, not the football validity of the resulting segments; smoothing, common radial scale, minimum duration, support exclusions, and acceptance gates remain explicit project choices.

After that protocol produced a B result dominated by minimum-duration fragmentation and sampling-frequency instability, a targeted representation fork compared discrete geometry with continuous local descriptions. Football precedent already includes time-varying player-team relative phase (Duarte et al., 2013), spatiotemporal velocity correlation (Marcelino et al., 2020), and one-second velocity/heading-change summaries (Kai et al., 2021). General movement-path work treats displacement, speed, turn, and straightness as stepwise or multi-step signals and explicitly permits moving-window, multiscale description without behavioral segmentation (Edelhoff, Signer, and Balkenhol, 2016). Rodríguez-Sánchez et al. (2026) provide a recent football example of continuous velocity-field representation at a collective spatial scale.
...
## Local Defensive Response Form Prior Art

Directional and radial attacker–defender geometry has substantial precedent. Vilar et al. (2014) use relative phase among futsal dyadic distances; Caetano et al. (2023) use vector-coded opposing-player coordination; Narizuka and Yamazaki (2016) quantify nearest-opponent velocity alignment; Chacoma et al. (2022) and Calero-Sanz et al. (2026) construct proximity/alignment marking-network views; and Yamazaki et al. (2026, preprint) study relative-speed pursuit geometry. Link et al. (2016), Forcher et al. (2024), and Wu and Swartz (2023) provide pressure and two-dimensional defensive-velocity neighbors.

These sources establish geometric method families, not this project's football interpretation. The [targeted methodology note](local_defensive_response_form_methodology.md) records what transfers and what does not. The frozen v1 protocol preserves focal-relative path, separates signed projections from magnitude, and does not import relative-phase categories, marking assignments, pressure, pursuit mechanisms, intent, or value.

## Concurrent Attacker–Defensive Geometry Prior Art

The concurrent v1 design is not a claim to have invented player co-movement, fixed tracking windows, dyadic geometry, or team shape. Vilar et al. (2014) provide relative-phase futsal precedent; Caetano et al. (2023) provide official-match nearest-opponent vector coding; and Moura et al. (2016) combine cross-correlation and vector coding for football player distributions. Centroid, stretch, pairwise-distance, expected-position, and role-conditioned defence research supplies further established ingredients. The [prospective provenance note](concurrent_attacker_defensive_geometry_v1_provenance.md) records the transfer boundary: v1's potentially differentiated contribution is the governed combination of concurrent attacker path, validated focal-relative path, closed defensive deformation, start-fixed rank localization, strictly prior activity context, and heldout separation before football interpretation.

## Opportunity Redistribution Prior Art

The [focused Opportunity Redistribution v1 audit](opportunity_redistribution_v1_methodology.md) distinguishes simple defender separation from richer meanings of opportunity. Link, Lang, and Seidenschwarz (2016) use defender distance in a ball-carrier pressure model; Dick, Link, and Brefeld (2022) model receiver availability using ball/player dynamics, interception, and technical skill; Spearman (2018) combines potential pitch control, transition, and scoring; and Fernández and Bornn (2018) define valued space occupation and teammate space generation. Bischofberger and Baca (2026) further warn that space-control meanings and validation differ substantially.

These sources establish separation, availability, control, and valued space as prior art. V1 deliberately selects only differential change in nearest-defender separation for non-focal attackers, with prospectively fixed recipient groups and no claim of pass availability, control, causation, or value. Potential novelty lies only in the governed connection to replicated focal-relative defensive geometry and later heldout validation—not invention of opportunity or space creation.
