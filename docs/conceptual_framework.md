...
> **Current status:** focal-relative path has passed held-out Metrica validation and external geometric replication across seven IDSSE matches from one independent provider environment. Phase 5A establishes contextual-prediction feasibility, driven primarily by recent focal movement. Phase 5B finds only a small, mixed opponent-information increment. Signed displacement is retained descriptively; continuation innovation is not a validated onset measure; and discrete attacker segmentation remains mixed. Continuous attacker geometry replicated across both Metrica sample matches. The frozen within-provider bridge and spatial footprint classified Final A, and a later concurrent design replicated a stronger focal-relative defender-movement association among near than middle ranks after pre-interval conditioning. Opportunity Redistribution v1 then tested a specific downstream implication and classified **Game 1 development negative**: $\beta_D=-0.02407$ [−0.09392, 0.04776], with two of three robustness signs also negative. The evidence supports localized defensive geometry, not teammate separation, space creation, causation, attribution, tactical success, gravity, or value. Historical sections preserve ideas later weakened or rejected and should be read with the [claim-status ledger](claim_status.md).
...
The latest evidence sharpens those conditional arrows. Replicated concurrent localization shows that nearby defenders' focal-relative movement changes more strongly with attacker movement than middle-rank defenders' movement under the frozen model. Local deformation supplies further geometry, but its timing-specific replication is weaker. Opportunity Redistribution v1 does **not** support equating either result with improved nearest-defender separation for other attackers: its primary coefficient was negative and uncertain, its three-nearest robustness interval was below zero, and the frozen trim remained negative. The weakly positive fixed-start result and positive but uncertain deformation alternative reinforce that downstream meaning is representation-sensitive. This is a genuine interpretation boundary, not an invitation to tune the nearest-defender metric until positive.
