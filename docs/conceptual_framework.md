...
> **Current status:** focal-relative path has passed held-out Metrica validation and external geometric replication across seven IDSSE matches from one independent provider environment. Phase 5A establishes contextual-prediction feasibility, driven primarily by recent focal movement. Phase 5B finds only a small, mixed opponent-information increment. Signed displacement is retained descriptively; continuation innovation is not a validated onset measure; and discrete attacker segmentation remains mixed. Attacker Movement Episode v2 reproduced its numerical fragmentation/merging/coverage trade-off on heldout Game 2 but failed a prospectively selected direction-boundary audit. Continuous attacker geometry replicated across both Metrica sample matches, and the prospectively frozen within-provider bridge classified Final A: preceding attacker path was associated with subsequent local focal-relative defensive movement beyond strictly earlier defensive context. This remains observational geometry. Tactical defensive response, causation, attribution, value, and relational reconfiguration remain unvalidated. Historical sections preserve ideas later weakened or rejected and should be read with the [claim-status ledger](claim_status.md).
