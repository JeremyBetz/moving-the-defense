...
The subsequent [representation fork](attacking_movement_representation_fork.md) selects **continuous fixed-window attacker geometry**. [Protocol v1.0](protocols/attacking_continuous_movement_v1.md) retained signed displacement, path length, and derived straightness over a 2 s primary window with 1/4 s sensitivities; heading change and speed variation were deferred. The [Game 1 execution](results/attacking_continuous_movement_game1_v1.md) is **A**: hard QC and all 15 frozen 25/10 Hz gates pass with byte-identical reproduction. Under the [held-out Game 2 protocol](protocols/attacking_continuous_movement_game2_heldout_v1.md), the mandatory [Stage-A raw trajectory-support audit](results/attacking_continuous_movement_game2_stage_a.md) is **READY**. Its deterministic registry freezes 138 intervals and leaves 2,093,028 raw outfield rows in 134 support segments. The next legitimate task is the unchanged Game 2 representation execution using that registry—not support revision or bridge analysis.
