...
The remaining choice is between consolidating the measurement paper and pursuing a separately motivated downstream construct. No Opportunity Redistribution v2, alternative opportunity metric, Game 2 opportunity execution, or tactical/value interpretation is authorized by the external result.

A design-only [Concurrent Defensive Coordination Form measurement review](concurrent_defensive_coordination_form_measurement_validation.md) now asks whether the replicated scalar localization has a component aligned with the attacker's changing local path. The local-increment algebra and physical-time filtering candidate passed synthetic checks, but the [prospective experiment](protocols/concurrent_defensive_coordination_form_v1_draft.md) is not frozen and no match-level scientific outcome has been computed. Continuous-block/filter-edge eligibility, inference, and classification rules require prospective review before Game 1 execution.
