...
The subsequent [representation fork](attacking_movement_representation_fork.md) selects **continuous fixed-window attacker geometry** as the next primary candidate. [Protocol v1.0](protocols/attacking_continuous_movement_v1.md) is now frozen before execution: signed displacement, path length, and derived straightness are retained over a 2 s primary window with 1/4 s sensitivities; heading change and speed variation are deferred. The next legitimate task is to execute that outcome-blind Game 1 protocol exactly. Only A permits a separately frozen Game 2 evaluation; no result permits an attacker-to-defender bridge in the same pass.
