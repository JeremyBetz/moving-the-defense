...
The next downstream design was subjected to a pre-execution adversarial review. [Defensive Coverage Redistribution v1](protocols/defensive_coverage_redistribution_v1_rejection.md) was rejected before any sample or outcome because repeated focal exclusion plus within-anchor demeaning did not identify the intended other-attacker construct. Its frozen artifacts remain unchanged. The [superseding v2 protocol](protocols/defensive_coverage_redistribution_v2.md) narrows the physical unit to one anchor, one start-defined ball-nearest reference attacker and one fixed other-nine matching set. Eighteen synthetic tests pass, including compensation, reference-only change and a magnitude-preserving internal-motion null. V2 remains unexecuted matching geometry—not validated coverage or opportunity—and Game 1 requires separate authorization. Game 2, Game 3, IDSSE coverage outcomes, tactical meaning and value remain unopened.
