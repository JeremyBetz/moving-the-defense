...
The next downstream construct was selected only after a separate literature and method audit. [Defensive Coverage Redistribution v1](protocols/defensive_coverage_redistribution_v1.md) is now prospectively frozen but unexecuted. It replaces independent nearest-defender separation with a joint minimum-cost matching over all nine non-focal attackers and ten outfield defenders, explicitly allowing a teammate to compensate when another defender leaves. The matching is a geometric capacity representation, not marking or assignment. Game 1 development requires separate authorization; Game 2, Game 3, IDSSE coverage outcomes, tactical meaning and value remain unopened.
