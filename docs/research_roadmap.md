...
The next downstream design was subjected to a pre-execution adversarial review. [Defensive Coverage Redistribution v1](protocols/defensive_coverage_redistribution_v1_rejection.md) was rejected before any sample or outcome because repeated focal exclusion plus within-anchor demeaning did not identify the intended other-attacker construct. Its frozen artifacts remain unchanged. The [superseding v2 protocol](protocols/defensive_coverage_redistribution_v2.md) narrowed the physical unit to one anchor, one start-defined ball-nearest reference attacker and one fixed other-nine matching set. Its [Game 1 closure](results/defensive_coverage_redistribution_game1_v2.md) is **INVALID before estimation**: complete support retained 281 period-1 anchors but no period-2 anchors, so the mandatory period-2 indicator was constant and the frozen 12-column model had rank 11. No coefficient, interval, null, or coverage conclusion exists. The [prospective v3 protocol](protocols/defensive_coverage_redistribution_v3.md) now freezes a single estimability remedy: omit only an explicitly designated non-scientific nuisance indicator when exactly constant over the complete eligible sample, then keep that active column set fixed. The separate outcome-blind rank-composition audit is complete at **MODERATE**. Its downstream decision is **V3 MAY PROCEED WITH A PAPER LIMITATION / NONCLASSIFYING QC**; no prospective core sensitivity is required. V3 has not executed, and Game 2, Game 3, IDSSE coverage outcomes, tactical meaning and value remain unopened.
