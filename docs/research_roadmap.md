...
The [local defensive response form v1 protocol](protocols/local_defensive_response_form_v1.md) preserves the validated movement magnitude and separately tests signed endpoint geometry. Its [Game 1 development execution](results/local_defensive_response_form_game1_v1.md) is **COHERENT**: near-minus-middle attacker-direction structure and its paired temporal-control difference passed every frozen gate. Game 2 is now conditionally available for unchanged internal replication but remains unexecuted for this estimand; Game 3 remains reserved. Radial, orthogonal, and absolute-versus-unit views remain descriptive rather than tactical labels.
