...
- **C — External-validation path:** the IDSSE replication design is now frozen but unexecuted; execution would test cross-provider transport of the established concurrent geometry before another Metrica opportunity experiment.

This remains a scientific decision point. The IDSSE concurrent-geometry protocol is frozen, but this design checkpoint does not authorize execution. No Opportunity Redistribution v2, alternative opportunity metric, Game 2 opportunity execution, or IDSSE result has been produced.
