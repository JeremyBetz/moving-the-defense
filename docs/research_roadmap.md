...
The subsequent [representation fork](attacking_movement_representation_fork.md) selects **continuous fixed-window attacker geometry**. [Protocol v1.0](protocols/attacking_continuous_movement_v1.md) retained signed displacement, path length, and derived straightness over a 2 s primary window with 1/4 s sensitivities; heading change and speed variation were deferred. The representation classified **A** on [Game 1](results/attacking_continuous_movement_game1_v1.md) and, under a separately frozen protocol and Stage-A registry, **A** on [held-out Game 2](results/attacking_continuous_movement_game2_v1.md). Hard QC and every frozen 25/10 Hz gate passed at both matches with byte-identical reproduction. The within-provider prerequisite is met; the next legitimate scientific task is a separately prospective attacker-to-defender bridge protocol, not reinterpretation or tuning of this representation.

That next design is now frozen in [attacker-to-defender bridge protocol v1.0](protocols/attacker_defender_bridge_v1.md), classified **READY before execution**. It tests whether attacker path over $[t-2,t]$ is associated with mean focal-relative path among the three nearest defenders over $[t,t+2]$, conditional on strictly earlier defensive movement. A four-second cadence, farthest-three control, reverse-time placebo, blocked uncertainty, and final two-match A/B/C rule are fixed. A pre-execution clarification adds exact Game 1 COHERENT/MIXED/INVALID status and terminal-block/bootstrap mechanics without changing the science. No bridge result exists; the next legitimate task is Game 1 implementation/execution, not protocol revision or Game 2 inspection.
